...
# include "/usr/XML/include/NinoIn.h"
...
mixed convert(mixed content, varargs string ltype, int strip) {
   ...
   */
   ...
   if (ltype == XML_PCDATA) {
	 ...
	 /* unless it's already e.g. COL_ELEMENT, paint it PCDATA */
	 content = paint_value(content, COL_PCDATA);
	 if (strip) {
   ...
   if (query_type_colour(ltype)) {
      ...
      return paint_value(content, query_type_colour(ltype));
   }
