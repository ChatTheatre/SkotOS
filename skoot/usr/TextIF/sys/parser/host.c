...
int confirm_changes() {
	...
	DEBUG("confirm_changes/host: query_all_adverbs() changed.");
	return TRUE;
    }
    ...
    if (sizeof(ix - old) || sizeof(old - ix)) {
	DEBUG("confirm_changes/host: query_verbs() changed.");
	return TRUE;
    }
    ...
    ix -= ({ nil });
    if (sizeof(ix - old) || sizeof(old - ix)) {
	DEBUG("confirm_changes/host: raw verbs changed.");
	return TRUE;
    }
    return FALSE;
}
...
void read_grammar(varargs int verify) {
	...
	*/
       if (!confirm_changes()) {
	   DEBUG("read_grammar/host: Nothing noticable changed, ignoring call.");
	   return;
       }
    }
    ...
    SysLog("Reading host.y file...");
    grammar = read_file("~/grammar/host.y");
    ...
    this_object()->destroy_parse_object();

    SysLog("Loaded host.y file.");
}
