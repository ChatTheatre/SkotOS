...
static
string list_people(object user, object body, varargs string str, string mode,
		   int hide) {
   mapping list, ip;
   object master, ob, who;
   object *users, *tmp, *bodies;
   int i, j, users_size, names_size, tmp_size, found;
   string name, *names, out, **arr;
   ...
   for(i = 0; i < names_size; i++) {
	 ...
	 found = FALSE;
	 tmp = list[names[i]];
	 tmp_size = sizeof(list[names[i]]);

	 if (names[i] == str)
	    found = TRUE;

	 for(j = 0; j < tmp_size; j++) {
	    if (contains(lower_case(describe_one(tmp[j])),
			 lower_case(str))) {
	       found = TRUE;
	    }
	 }
	 ...
	 arr[0] += ({ names[i] });
	 arr[1] += ({ describe_one(bodies[j]) });
	 if (bodies[j]->query_environment()) {
	    arr[2] += ({
		hide ? describe_one(bodies[j]->query_environment()) :
		       name(bodies[j]->query_environment()) });
	 } else {
	    arr[2] += ({ "-" });
	 }
      }
   ...
   if ( mode=="all" ) {
	 ...
	 out += " user and ";
      out += users_size + "";
      if (users_size > 1)
	 out += " characters online.\n";
      else
	 out += " character online.\n";
   } else {
      out += names_size + "";
      if ( names_size > 1 )
	 out += " "+mode+" online.\n";
